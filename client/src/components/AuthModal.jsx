...
const AuthModal = ({ isOpen, onClose, onAuthSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0D0D0D]/60 backdrop-blur-xs overflow-y-auto">
      <div className="relative w-full max-w-md bg-white border border-[#D6D3CF] rounded-2xl p-6 sm:p-8 shadow-card-hover my-8">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 text-[#9A9A9A] hover:text-[#0D0D0D] bg-slate-100 hover:bg-slate-200 rounded-full transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-xl bg-white border border-[#D6D3CF] mb-2 shadow-sm p-1">
            <img src="/coders-club-logo.png" alt="Coders' Club Logo" className="w-full h-full object-contain" />
          </div>
          <h3 className="text-2xl font-bold text-[#0D0D0D]">CODEX 4.0 PORTAL</h3>
          <p className="text-xs text-[#E64B2E] font-medium mt-1">Coders' Club Flagship Event</p>
        </div>
        ...
        <div className="relative flex items-center justify-center mb-5">
          ...
          <span className="bg-white px-3 text-[10px] font-semibold text-[#9A9A9A] uppercase shrink-0">
            or email
          </span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {!isLogin && (
            <div className="grid grid-cols-2 gap-2 pt-1">
              <div>
                <label className="block text-[10px] font-semibold text-[#9A9A9A] uppercase mb-1">
                  Roll No
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-2.5 py-2 rounded-lg bg-white border border-[#D6D3CF] text-[#0D0D0D] text-xs uppercase"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-2.5 py-2 rounded-lg bg-white border border-[#D6D3CF] text-[#0D0D0D] text-xs"
                />
              </div>
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 rounded-xl bg-[#E64B2E] hover:bg-[#c73d21] text-white font-semibold text-xs shadow-sm transition-all mt-3"
          >
